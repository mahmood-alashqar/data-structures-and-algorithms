...
const maxInArray = (arr) => {
  // Solution code here...
  const maximumValue = arr.reduce((max , value) => {
    if(max < value )
    {
      max = value ;
    }
...
const checkValues = (obj, value) => {
 ...
 let checkValues = true;
  Object.values(obj).forEach(objValue => {
    if(objValue !== value) checkValues=false;
  } );
  return checkValues ;
...
const updateNumbers = (obj) => {
  // Solution code here...
  const FinalDateShape = Object.keys(obj).map(key => `${String(key)}: ${String(obj[key])}` );
  return FinalDateShape ;
};
...
const hasChildrenValues = (arr, character) => {
  let checkIfExisit = false ;
  arr.forEach(obj => {
    if(Object.values(obj).indexOf(character) !== -1)
      if(Object.keys(obj).indexOf('children') !== -1)  checkIfExisit=true;
  } );
  return checkIfExisit ;
  // Solution code here...
