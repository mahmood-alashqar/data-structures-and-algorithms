...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  starWarsArr.sort((firstCompare , secondCompare) => Number(secondCompare.height) - Number(firstCompare.height) );
  return starWarsArr ;
}
...
let arrayBelongToChallenge7 = [];
const listFoods = (recipe) => {
  // let result = [];
  // result = recipe.ingredients.map(item =>{
  //    item.slice(item.indexOf(' ', item.indexOf(' ')+1)+1)
// arrayBelongToChallenge7.push(item);
// console.log(arrayBelongToChallenge7);
let result = [];
let numberOfSpace = 2;
 recipe.ingredients.map(item =>{
  let a = item.split(' ');
   let final =item.slice(numberOfSpace).join(' ');
   result.push(final);
