...
const longestString = (arr) => {
  let longestString = -1;
  arr.reduce((compareLongestString, str, index) => {
    if (compareLongestString.length < str.length) {
      longestString = index;
      compareLongestString = str;
    }
    return compareLongestString;
...
const standardizePhoneNumbers = (arr) => {
  const numberFormat = arr.map(phoneNum => phoneNum.match(/[0-9]/g).join(''));
  return numberFormat;
};
...
const onlyOddChars = (str) => {
  const result = (str.split('')).filter((char, idx) => idx % 2 !== 0 ? true : false).join('');
  return result;
};
