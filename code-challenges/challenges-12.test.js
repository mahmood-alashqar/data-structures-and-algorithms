...
const replaceZeros = (string) => {
  // Solution code here...
  const reg = /0/g;
  return string.replace(reg, 'zero');// string methode in JS to replace 
}
...
const validatePin = (pin) => {
  // Solution code here...
  const numbercount = String(pin).match(/([0-9])/g);
  const nan = String(pin).match(/([^0-9])/g);
  return numbercount ? ((numbercount.length === 4 && !nan) ? true : false) : false;
};
...
const validateWord = (word) => {
  ...
  const numberOfChart = word.match(/\w/g);
  return numberOfChart ? ((numberOfChart.length >= 5 && numberOfChart.length < 10) ? true : false) : false;
...
const findTagNames = elements => {
  // Solution code here...
  const regex = /<(\/[a-z]+\d?)>/g;
  return elements.map(string => string.match(regex)).reduce((arr, result) => arr.concat(result.map(result => result.slice(1, (result.length - 1)))), []);
};
