...
const sortByChildren = (charArray) => {
  // Solution code here...
  charArray.sort((numberOfChildrens_1 , numberOfChildrens_2) => {
    if((numberOfChildrens_1.children.length - numberOfChildrens_2.children.length) !== 0 )
      return (numberOfChildrens_1.children.length - numberOfChildrens_2.children.length) ;
    else {
      for(let i = 0 ; i<numberOfChildrens_1.house.length ; i++ )
        if((numberOfChildrens_1.house[i].charCodeAt() - numberOfChildrens_1.house[i].charCodeAt()) !== 0)
          return numberOfChildrens_1.house[i].charCodeAt() - numberOfChildrens_1.house[i].charCodeAt() ;
    }
    return 0 ;
  }
  );
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  const results = arr.filter(cityName => {
    cityName[0].match(/[A-J]/)});
  return results ;
};
